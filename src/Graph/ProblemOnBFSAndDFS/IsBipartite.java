...
public class IsBipartite {
    ...
    public static class Solution {
        ...
        // Colors the graph using BFS and checks if it is bipartite
        private static boolean colorGraph(int startVertex) {
            // Assign the first color (1) to the starting vertex
            vertexColors[startVertex] = 1;
            ...
            while (!queue.isEmpty()) {
                ...
                // Get the current vertex's color
                int currentColor = vertexColors[currentVertex];
                // Alternate color for neighbors (1 -> 2, 2 -> 1)
                int alternateColor = currentColor == 1 ? 2 : 1;
                ...
                for (int i = 0; i < graph.get(currentVertex).size(); i++) {
                    ...
                    if (vertexColors[neighbor] == currentColor) {
                        return false;
                    }

                    // If neighbor is uncolored, assign alternate color and add to queue
                    if (vertexColors[neighbor] == 0) {
                        vertexColors[neighbor] = alternateColor;
                        queue.add(neighbor);
                    }
                }
            }
